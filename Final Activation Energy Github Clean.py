...
##mp.prec = 10 #This is the number of digits mpf's are calculated to, incresing this will increase the evaluation time
#mp.dps = 150    
...
def temprhs(z, T, *targs):
    Con1, Con2, Con3, Twall, Ra = targs
    EQ1 = T[1]/10000000.0
    EQ2 = ((Con1*T[1])/10000000.0) + (((Con2*(Twall - T[0]))*(1.0)) + (Con3*(Ra/1000000.0)))
    return [EQ1,EQ2]
    
def temprhsD(z, T, *targs):
    Con1, Con2, Con3, Twall, Ra = targs
    EQ1 = Decimal(T[1])/Decimal(100000000.0)
    EQ2 = ((Decimal(Con1)*Decimal(T[1]))/Decimal(100000000.0)) + (((Decimal(Con3)*(Decimal(Ra)/Decimal(1000000.0))) - (Decimal(Con2)*(Decimal(T[0]) - Decimal(Twall)))*Decimal(1.0)))
    return [EQ1,EQ2]


def tempjacob(z, T, *targsD):
    con1, con2, con3, twall, ra = targsD
    T0 = T[0] #sp.symbols('T0')
    T1 = T[1] #sp.symbols('T1')
    EQ1 = T1/100000000.0
    EQ2b = ((con1*T1)/100000000.0) + ((con3*(ra/1000000.0)) - ((con2*(T0 - twall))*(1.0)))
    ttdot = [EQ1,EQ2b]
    Tm2 = sp.Matrix(ttdot).jacobian(T)
    Tm2.subs({'con1': con1, 'con2': con2, 'con3': con3, 'twall' : twall, 'ra' : ra})
    Tj2 = sp.lambdify((z, T) , Tm2)
    return Tj2
...
initchem = tc.Chemical('107-06-2',T=Temp_K, P=PascalP)
#print(initchem.rhog*(1000.0/98.95))
#print(initchem.rhog/mwedckg)
edcmw = begmix.MW
Rvalc = begmix.R_specific
rhoino = begmix.rho #[kg/m^3]
rhoin = rhoino*(1E3)*(1E-6)
rhoin2 = rhoin/edcmw
rhoing = begmix.rhogm*1000.0 #[mol/m^3]
...
u_zseg = u_z/segment_second
eos = tc.PR(Tc=Tcedc, Pc=Pcedc, omega=omegaedc, T=Temp_K, P=PascalP)
#print(eos.V_g)
#print(1.0/eos.V_g)
gnodes = 1
gnodes2 = 1
graph_num = 1
Ls2 = L/(desired_time*segment_second)
alphaint = (math.pi*2.0*ri*Ls2)/(Ac*Ls2)
...
#vflow_cmf = vflow_cm/segment_second #[cm^3/s]
...
tau2 = intvol/u_z
tau4 = intvols/u_z
taui = 1.0/segmentlength
tauib = totalvolumeint/(0.1*u_z)
tauic = tauib/Ls
tauid = taui/L

#print(taui,tau3,tau4)
taui2 = 1.0
#print('Seconds: {}, Seconds^-1 {}'.format(tau,tauinv))
...
#for il in range(iternum):
    ...
    bedc = Bviral3(Temp_K,Tcedc,Pcedc,omegaedc)
    zedc = btoz(bedc,Temp_K,PascalP)
    initedc = PascalP/(Temp_K*Rr*zedc) #rhoinit
    EDC = [Decimal(initedc)]
    EDCj = [Decimal(initedc)]
    Temp_vals.append(amount_new)
    Temp_vals2.insert(0,amount_new)
    Eabb = [amount_new,7.0,42.0,45.0,34.0,48.0,13.0,12.0,4.0,6.0,15.0,0.0,56.0,31.0,30.0,61.0,84.0,90.0,70.0,20.0,70.0] #[kJ/mol]
    Eab = [x*1000.0 for x in Eabb]
    Ea = [Decimal(x) for x in Eab] #kJ/mol
    Eaff = [i for i in Ea]
    ks = [5.9E+15,1.3E13,1E+12,5E+11,1.2E+13,2E+11,1E+13,1E+13,1.7E+13,1.2E+13,1.7E+13,91000000000,1.2E+14,5E+11,20000000000,3E+11,2.1E+14,5E+14,2E+13,1E+14,1.6E+14]
    n = [1,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2]
    k_0b = [i/(1E6**(int(j-1))) for i,j in zip(ks,n)]
    k_0 = [De(i) for i in k_0b]
...
#    for x2 in tqdm(dist_c):
        ...
        gmixw2 = tc.Mixture(IDs = names2b, zs=C_i2, T=float(Twall), P=PascalP)
        vsb = gmixw2.mug # [Pa*s]
        vslist = gmixw2.mugs # [Pa*s]
        vs = tc.viscosity.Brokaw(T=float(Twall),ys=C_i2,mus=viss,MWs=mwss,molecular_diameters=sigma,Stockmayers=stemix) # [Pa*S]
        pr = Pr(cpkgmix,viscosity,kval2)
        re = reynolds(rhob,u_z,Ls,viscosity)
        nuval = Nus(velocity,rhoc,Ls,diameter,kval2,viscosity,vs,float(Twall),float(T0[-1]),pr,re)
        gash = hterm(nuval,Ls,kval2)
        h20 = tc.Chemical('7732-18-5', T=float(Twall), P=PascalP)
        h20cp = h20.Cpgm
        ...
        h20wall = tc.Chemical('7732-18-5', T=float(Twall), P=PascalP)
        h20vsb = h20wall.mug # [Pa*s]
        h20vs = h20vsb # [g/m*s^2]
        hnuval = Nus(velocity,h20rho,Ls,diameter,h20k,h20vis,h20vs,float(Twall),float(Twall),prh20,reh20)
        ...
        con2 = (alpha*U_coeff)/kval2 # [1/m^2] (alpha) 
        con3 = delhm/kval2 # -> [s*m*K/mol]  vintflowf*
        RE_vals.append(re)
        ...
        Tstarj = tc.lennard_jones.Tstar(T=float(T0j[-1]), epsilon_k=ekj, epsilon=None)
        colintj = tc.lennard_jones.collision_integral_Kim_Monroe(Tstarj, l=1, s=1)
        gmixw2j = tc.Mixture(IDs = names2bj, zs=C_i2j, T=float(Twall), P=PascalP)
        vsj = gmixw2j.mug # [g/cm*s^2]
        vs2j = tc.viscosity.Brokaw(T=float(Twall),ys=C_i2j,mus=vissj,MWs=mwssj,molecular_diameters=sigmaj,Stockmayers=stemixj)
        prj = Pr(cpgmmixj,vsj,kval2j)
        rej = reynolds(rhoj,velocityj,distances,vsj)
        nuvalj = Nus(velocity,rhoj,distances,diameter,kval2j,vsj,vs2j,float(Twall),T0j[-1],prj,rej)
        gashj = hterm(nuvalj,distances,kval2j)
        h20j = tc.Chemical('7732-18-5', T=float(Twall), P=PascalP)
        h20cpj = h20j.Cpgm
        ...
        h20wallj = tc.Chemical('7732-18-5', T=float(Twall), P=PascalP)
        h20vsj = h20wallj.mug # [g/cm*s^2]
        hnuvalj = Nus(velocityj,h20rhoj,distances,diameter,h20kj,h20visj,h20vsj,float(Twall),float(Twall),prh20j,reh20j)
        ...
        Y02ra = [EDC[-1],EC[-1],HCl[-1],Coke[-1],CP[-1],Di[-1],C4H6Cl2[-1],C6H6[-1],C2H2[-1],C11[-1],C112[-1],R1[-1],R2[-1],R3[-1],R4[-1],R5[-1],R6[-1],VCM[-1]]
        ...
        args1 = [De(c1_vals[-1]),De(c2_vals[-1]),De(c3_vals[-1]),k_0, De(Twall)]
        argsj = [float(c1_valsj[-1]),float(c2_valsj[-1]),float(c3_valsj[-1]),k_0b, float(Twall)]
        argsjD = [De(c1_valsj[-1]),De(c2_valsj[-1]),De(c3_valsj[-1]),k_0, De(Twall)]
        args1diff = [float(c1_valsj[-1]),float(c2_valsj[-1]),float(c3_valsj[-1]),k_0b, float(Twall)]
        args1diff2 = [De(c1_valsj[-1]),De(c2_valsj[-1]),De(c3_valsj[-1]),k_0, De(Twall)]
        getcontext().prec = 50
        precision = 50
        res = solve_ivp(RHS, [0.0,Ls], Y02 , method = 'Radau',  args=(8.314, 5.9E15, 13000000.0, 1000000.0, 500000.0, 12000000.0, 200000.0, 10000000.0, 10000000.0, 17000000.0, 12000000.0, 17000000.0, 91000.0, 120000000.0, 500000.0, 20000.0, 300000.0, 2.1E+8,5E+8,2E+7, 100000000.0, 160000000.0, amount_new*1000.0, 7000.0, 42000.0, 45000.0, 34000.0, 48000.0, 13000.0, 12000.0, 4000.0, 6000.0, 15000.0, 0.0, 56000.0, 31000.0, 30000.0, 61000.0, 84000.0, 90000.0, 70000.0, 20000.0, 70000.0, float(c1_valsj[-1]),float(c2_valsj[-1]),float(c3_valsj[-1])), jac=jacob)
...
#    conversion_EDCb = [(1-Decimal(x)/initial_edc)*Decimal(100) for x in EDC] #Calculates the conversion of EDC (% basis) at each time interr
#    conversion_EDCbj = [(1-Decimal(y)/initial_edc)*Decimal(100) for y in EDCj]
...
np.savetxt(r"{}\Conversion_EDC.txt".format(dir_path),convedc)
edcj = np.array(EDClj)
...
#        
#        
#        plt.plot(xD, cur_list2, color=viridis.colors[index_jj,:], label=r"Ea Value: {} [$\frac{}{}$]".format(eavalg,str('{kJ}'),str('{mol}')))
#        plt.legend(loc='best')
#        plt.grid()
#        plt.xlabel(r'Distance [$m$]')
#        plt.ylabel(r'Concentration [$\frac{mol}{m^3}$]')
#        plt.title('{} Concentration'.format(nameD2))
#        fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\{}-Concentration NoJ-{}.pdf".format(nameD,eavalg))
#        fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\{}-Concentration NoJ-{}.svg".format(nameD,eavalg))
#
...
#        plt.ylabel(r'Concentration [$\frac{mol}{m^3}$]')
#        plt.title('{} Concentration'.format(nameD2))
#        fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\{}-Concentration J-{}.pdf".format(nameD,eavalg))
#        fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\{}-Concentration J-{}.svg".format(nameD,eavalg))
#
#
#
#font = {'family': 'serif',
...
#plt.grid()
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDCconcentration.png") #Saves the graph
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDCconcentration.svg")
#
#fig, ax = plt.subplots()
#ax.plot(tvals, eaendc, 'b-', label='Conversion')
#ax.plot(tvals, eaendcj, 'r-', label='Conversion With Jacobian')
#ax.set(xlabel=r"Inital Ea Value [$\frac{}{}$]".format(str('{kJ}'),str('{mol}')), ylabel='Conversion [%]', title="Final Conversion")
#ax.legend()
#ax.grid()
#ax.invert_xaxis()
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Final Conversionback.pdf", bbox_inches='tight')
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Final Conversionback.svg", bbox_inches='tight')
#
#fig = plt.figure()
#plt1e = plt.plot(tvals, eaendc, 'b-')
#plt2e = plt.plot(tvals, eaendcj, 'g-')
#plt.ylabel('Conversion')
#plt.xlabel(r"Inital Ea Value [$\frac{}{}$]".format(str('{kJ}'),str('{mol}')))
...
#plt.grid()
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Final Conversion.png") #Saves the graph
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Final Conversion.svg")
#
#fig = plt.figure()
...
#    
#    
#    plt.plot(xD, edc[i, :], color=viridis.colors[index_i, :], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[i],str('{kJ}'),str('{mol}')))
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC-NoJ.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC-NoJ.svg")
#
#fig = plt.figure()
#for ii,i in enumerate(graph_nodes2):
#    index_i = int(i) 
#    
#    
#    plt.plot(xD, edcj[i, :], color=viridis.colors[index_i, :], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[i],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
#plt.ylabel(r'Concentration [$\frac{mol}{m^3}$]')
#plt.title('EDC Concentration')
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC-J.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC-J.svg")
#
#
...
#       
#       
#       plt.plot(xD, edcj[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       TemperatureC = KtoC(Temp_K)
#       plt.legend(loc='best')
#       plt.grid()
#       plt.xlabel(r'Distance [$m$]')
...
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC.pdf")
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC.svg")
#
#
#
#
...
#       
#       index_k = int(j)
#       plt.plot(timeD, convedcj[j, :], color=viridis.colors[index_k,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       TemperatureC = KtoC(Temp_K)
#       plt.legend(loc='best')
...
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC-conv.pdf")
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EDC-conv.svg")
#
#
#fig = plt.figure()
...
#    
#    
#    plt.plot(xD, ecj[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EC.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\EC.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#       
#       
#       plt.plot(xD, hclj[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\HCl.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\HCl.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, ccj[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#
#plt.grid()
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Coke.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Coke.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, cp1j[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#
#plt.grid()
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\CP.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\CP.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, dij[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Di.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Di.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, c4h6cl2j[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C4H6Cl2.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C4H6Cl2.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, c6h6j[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C6H6.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C6H6.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, c2h2j[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C2H2.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C2H2.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, c11j[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#
#plt.grid()
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C11.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C11.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#    
#    
#    plt.plot(xD, c112j[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#
#plt.grid()
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C112.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\C112.svg")
#
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
...
#       
#       
#       plt.plot(xD, vcmj[j, :], color=viridis.colors[index_j,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       TemperatureC = KtoC(Temp_K)
#plt.legend(loc='best')
#plt.grid()
#plt.xlabel(r'Distance [$m$]')
...
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\VCM.pdf")
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\VCM.svg")
#
#
#
#for jj,j in enumerate(graph_nodes):
...
#       
#       index_t = int(j)
#       plt.plot(xD, t0[j, :], color=viridis.colors[index_t,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       plt.legend(loc='best')
...
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\T-Profile{}.pdf".format(Temp_vals[j]))
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\T-Profile{}.svg".format(Temp_vals[j]))
#
#for jj,j in enumerate(graph_nodes):
#       fig = plt.figure()
#       
#       
#       index_t = int(j)
#       plt.plot(xD, t0j[j, :], color=viridis.colors[index_t,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       plt.legend(loc='best')
...
#       plt.ylabel('Temperature [K]')
#       plt.title('Temperature Profile')
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\T-Profile-J{}.pdf".format(Temp_vals[j]))
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\T-Profile-J{}.svg".format(Temp_vals[j]))
#
##for jj,j in enumerate(graph_nodes):
##       fig = plt.figure()
##       
##       
##       index_t = int(j)
##       plt.plot(xD, dt0[j, :], color=viridis.colors[index_t,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
##       plt.legend(loc='best')
...
##       plt.ylabel('Temperature [K]')
##       plt.title('Temperature Profile')
##       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\T-Profile Diffusion{}.pdf".format(Temp_vals[j]))
##       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\T-Profile Diffusion{}.svg".format(Temp_vals[j]))
#
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt.ylabel(r'Concentration [$\frac{mol}{m^3}$]')
#    plt.legend(loc='best')
#    plt.title(r"By-Product Concentration Ea: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.grid()
#     
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\RadicalT{}.pdf".format(Temp_vals[j]), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\RadicalT{}.svg".format(Temp_vals[j]), bbox_inches='tight')
#
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt.grid()
#     
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\By-ProductT{}.pdf".format(Temp_vals[j]), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\By-ProductT{}.svg".format(Temp_vals[j]), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    TemperatureC = KtoC(Temp_K)
...
#    plt.ylabel(r'Concentration [$\frac{mol}{m^3}$]')
#    plt.legend(loc='best')
#    plt.title(r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ProductsNoJT{}.pdf".format(Temp_vals[j]), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ProductsNoJT{}.svg".format(Temp_vals[j]), bbox_inches='tight')
#    
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt.title(r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ProductsJT{}.pdf".format(Temp_vals[j]), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ProductsJT{}.svg".format(Temp_vals[j]), bbox_inches='tight') 
#    
#   
#
#for jj,j in enumerate(graph_nodes):
...
#       
#       index_t = int(j)
#       plt.plot(xD, purejl[j, :], color=viridis.colors[index_t,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#       plt.legend(loc='best')
#       plt.grid()
#       plt.xlabel(r'Distance [$m$]')
...
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Temp_vals-PurityJ{}.pdf".format(Temp_vals[j]))
#       fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Temp_vals-PurityJ{}.svg".format(Temp_vals[j]))
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
#       index_t = int(j)
...
#plt.gca().set_yticklabels(['{:d}%'.format(int(x)) for x in plt.gca().get_yticks()]) 
#plt.title('Product Purity - Jacobian')
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\OverallTemp_vals-PurityJ{}.pdf".format(Temp_vals[j]))
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\OverallTemp_vals-PurityJ{}.svg".format(Temp_vals[j]))
#
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    
#    index_t = int(j)
#    plt.plot(xD, purel[j, :], color=viridis.colors[index_t,:], label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.legend(loc='best')
#    plt.grid()
#    plt.xlabel(r'Distance [$m$]')
#    plt.ylabel('Product Purity')
...
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Temp_vals-Purity{}.pdf".format(Temp_vals[j]))
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Temp_vals-Purity{}.svg".format(Temp_vals[j]))
#
#fig = plt.figure()
#for jj,j in enumerate(graph_nodes2):
#       index_t = int(j)
...
#plt.gca().set_yticklabels(['{:d}%'.format(int(x)) for x in plt.gca().get_yticks()]) 
#plt.title('Product Purity')
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\OverallTemp_vals-Purity{}.pdf".format(Temp_vals[j]))
#fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\OverallTemp_vals-Purity{}.svg".format(Temp_vals[j]))
#
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#
#    index_j = int(j)
#    plt.plot(xD, convedc[j, :], 'b-', label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.xlabel(r'Distance [$m$]')
#    plt.ylabel('Conversion')
#    plt.gca().set_yticklabels(['{:d}%'.format(int(x)) for x in plt.gca().get_yticks()]) 
#    plt.legend(loc='best')
...
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ConversionT{}.pdf".format(Temp_vals[j]), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ConversionT{}.svg".format(Temp_vals[j]), bbox_inches='tight')
#    
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt.plot(xD, convedcj[j, :], 'b-', label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.xlabel(r'Distance [$m$]')
#    plt.ylabel('Conversion')
#    plt.gca().set_yticklabels(['{:d}%'.format(int(x)) for x in plt.gca().get_yticks()]) 
#    plt.legend(loc='best')
#    plt.title("Conversion of EDC")
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ConversionJT{}.pdf".format(eavalg), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\ConversionJT{}.svg".format(eavalg), bbox_inches='tight')
#    
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
#    eavalg = str(Temp_vals[j])
#    plt.plot(xD[0:-1], kvalsj[j, :], 'b-', label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.xlabel(r'Distance [$m$]')
#    plt.ylabel(r'Thermal Conductivity [$\frac{W}{m * k}$]')
#    plt.legend(loc='best')
#    plt.title('Mixture Thermal Conductivity ($\kappa_{mix}$)')
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\KvalsJT{}.pdf".format(eavalg), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\KvalsJT{}.svg".format(eavalg), bbox_inches='tight')
#    
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
#    eavalg = str(Temp_vals[j])
#    plt.plot(xD[0:-1], uvalsj[j, :], 'b-', label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.xlabel(r'Distance [$m$]')
#    plt.ylabel(r'U [$\frac{W}{m^2 \cdot K}$]')
#    plt.legend(loc='best')
#    plt.title('Overall Heat Transfer Coefficient ($\it{U}$)')
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\UvalsJT{}.pdf".format(eavalg), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\UvalsJT{}.svg".format(eavalg), bbox_inches='tight')
#    
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
#    eavalg = str(Temp_vals[j])
#    plt.plot(xD[0:-1], hvalsj[j, :], 'b-', label=r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')))
#    plt.xlabel(r'Distance [$m$]')
#    plt.ylabel(r'h [$\frac{W}{m^2 \cdot K}$]')
#    plt.legend(loc='best')
#    plt.title('Heat Transfer Coefficient ($\it{h}$)')
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\HvalsJT{}.pdf".format(eavalg), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\HvalsJT{}.svg".format(eavalg), bbox_inches='tight')
#    
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$\frac{\it{u_{z}}*\rho*C_{p}}{\kappa}$"],loc='best',fontsize='large')
#    plt.title('Constant 1', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Constant1JT{}.pdf".format(eavalg))
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Constant1JT{}.svg".format(eavalg))
##    plt.draw()
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt2 = plt.plot(xD[0:-1], c2valsj[j][:], 'b-', label='Constant 2')
#    plt.xlabel(r'Distance [$m$]', fontdict=font)
#    plt.ylabel(r'Constant 2 [$\frac{1}{m^2}$]', fontdict=font)
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$\frac{\it{U}*\alpha}{\kappa}$"],loc='best',fontsize='large')
#    plt.title('Constant 2', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Constant2JT{}.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Constant2JT{}.svg".format(float(eavalg)), bbox_inches='tight')
#    
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
...
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$\frac{\Delta{H_{rxn}}}{\kappa}$"],loc='best',fontsize='large')
#    plt.title('Constant 3', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Constant3JT{}.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Constant3JT{}.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$\frac{\it{\rho}*\it{u_{z}}*L_{s}}{\it{\mu}}$"],loc='best',fontsize='large')
#    plt.title('Reynolds Number', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Reynolds Number {}K.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Reynolds Number {}K.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Reynolds Number J {}K.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Reynolds Number J {}K.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$S_{\frac{VCM}{HCl}}$"],loc='best',fontsize='large')
#    plt.title('Selectivity of Vinyl Chloride Monomer', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityVCM-T-{}.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityVCM-T-{}.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.axhline(y=float(1.0), color='k', linestyle='--')
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$S_{\frac{HCl}{VCM}}$"],loc='best',fontsize='large')
#    plt.title('Selectivity of Hydrogen Chloride', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityHCl-T-{}.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityHCl-T-{}.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityVCMJ-T-{}.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityVCMJ-T-{}.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityHClJ-T-{}.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\SelectivityHClJ-T-{}.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.legend([r"Ea Value: {} [$\frac{}{}$]".format(Temp_vals[j],str('{kJ}'),str('{mol}')),r"$Y_{VCM}$"],loc='best',fontsize='large')
#    plt.title('Yield of Vinyl Chloride Monomer', fontdict=font)
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Yield VCM {}K.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Yield VCM {}K.svg".format(float(eavalg)), bbox_inches='tight')
#
#for jj,j in enumerate(graph_nodes):
#    fig = plt.figure()
#    index_j = int(j)
...
#    plt.grid()
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Yield VCM J {}K.pdf".format(float(eavalg)), bbox_inches='tight')
#    fig.savefig(r"C:\Users\tjcze\Desktop\Thesis\Python\Simple\Ea-Values\Yield VCM J {}K.svg".format(float(eavalg)), bbox_inches='tight')
#
#
